...
function ImageStudio() {
  ...
  return (
    <div className="min-h-dvh bg-[#05050c] text-white/90 relative overflow-hidden">
      ...
      <header className="relative z-10 flex items-center justify-between border-b border-white/[0.06] px-4 py-3 sm:px-6"
        ...
        <div className="flex items-center gap-3">
          <Link
            to="/"
            className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] px-3 py-1.5 text-xs text-white/70 transition hover:border-white/20 hover:text-white"
          >
        ...
        <div className="hidden items-center gap-2 text-[11px] text-white/50 md:flex">
          <span className="inline-flex items-center gap-1.5 rounded-md border border-white/10 bg-black/30 px-2 py-1">
      ...
      <main className="relative z-10 mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[380px_1fr]">
        ...
        <section className="space-y-4">
          <div className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-4 backdrop-blur-xl">
            <label className="mb-2 block text-[11px] uppercase tracking-[0.2em] text-white/50">
              Prompt
            </label>
            <textarea
              ref={inputRef}
              ...
              rows={5}
              className="w-full resize-none rounded-lg border border-white/10 bg-black/40 px-3 py-2.5 text-sm text-white/90 placeholder:text-white/30 focus:border-white/20 focus:outline-none focus:ring-2 focus:ring-[color:var(--color-iris-cyan)]/30"
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {SUGGESTIONS.map((s) => (
                <button
                  key={s}
                  onClick={() => setPrompt(s)}
                  className="rounded-md border border-white/[0.08] bg-white/[0.02] px-2 py-1 text-[10px] text-white/50 transition hover:border-white/20 hover:text-white/80"
                >
                  {s.slice(0, 32)}…
                </button>
              ))}
          ...
          <div className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-4 backdrop-blur-xl space-y-4">
            <div>
              <label className="mb-2 block text-[11px] uppercase tracking-[0.2em] text-white/50">
                Model
              </label>
                ...
                {POLL_MODELS.map((m) => (
                  <button
                    ...
                    className={`rounded-lg border px-2.5 py-2 text-left text-xs transition ${
                      model === m.id
                        ? "border-[color:var(--color-iris-cyan)]/40 bg-[color:var(--color-iris-cyan)]/[0.08] text-white"
                        : "border-white/[0.08] bg-black/20 text-white/60 hover:border-white/20 hover:text-white/90"
                    }`}
                  >
                ...
                {POLL_RATIOS.map((r) => (
                  <button
                    ...
                    className={`rounded-md border px-2.5 py-1.5 text-[11px] transition ${
                      ratio === r.id
                        ? "border-[color:var(--color-iris-cyan)]/40 bg-[color:var(--color-iris-cyan)]/[0.08] text-white"
                        : "border-white/[0.08] bg-black/20 text-white/60 hover:border-white/20"
                    }`}
                  >
                    {r.id}
                  </button>
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                <label className="text-[11px] uppercase tracking-[0.2em] text-white/50">Seed</label>
                <button
              ...
              <input
                ...
                className="w-full rounded-lg border border-white/10 bg-black/40 px-3 py-1.5 font-mono text-xs text-white/80 focus:border-white/20 focus:outline-none"
              />
            </div>
            ...
            <label className="flex items-center gap-2 text-xs text-white/70 cursor-pointer">
              <input
                ...
                className="h-3.5 w-3.5 rounded border-white/20 bg-black/40 accent-[color:var(--color-iris-cyan)]"
              />
          ...
          >
            <span className="relative z-10 inline-flex items-center justify-center gap-2">
              ...
              <span className="ml-2 rounded-md bg-black/25 px-1.5 py-0.5 text-[10px] font-mono font-normal opacity-70">
                ⌘↵
              </span>
            </span>
          </button>
          ...
          <p className="text-center text-[10px] text-white/30">
            Powered by <span className="text-white/50">pollinations.ai</span> · Free & unlimited · No signup
          </p>
        </section>
        ...
        <section>
          {items.length === 0 ? (
            <div className="grid h-full min-h-[60vh] place-items-center rounded-2xl border border-dashed border-white/[0.08] bg-white/[0.01]">
              <div className="text-center">
                ...
                <p className="mt-2 max-w-md text-sm text-white/50">
                  Write a prompt on the left and hit <span className="font-mono text-white/70">Generate</span>.
...
function GalleryCard({
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-white/[0.08] bg-black/40 backdrop-blur-xl">
      <div className="relative w-full overflow-hidden" style={{ aspectRatio: aspect }}>
        {!loaded && !errored && (
          <div className="absolute inset-0 grid place-items-center">
            <div className="absolute inset-0 animate-pulse"
              style={{
                background:
                  "linear-gradient(135deg, color-mix(in oklab, var(--color-iris-deep) 30%, #05050c), color-mix(in oklab, var(--color-iris-cyan) 20%, #05050c))",
              }}
            />
            <div className="relative z-10 flex flex-col items-center gap-2">
              <Loader2 className="h-6 w-6 animate-spin text-white/70" />
              <span className="text-[10px] uppercase tracking-widest text-white/60">Rendering…</span>
            </div>
          </div>
        )}
        {errored && (
          <div className="absolute inset-0 grid place-items-center bg-black/60">
            <div className="text-center text-xs text-white/60">
              <p>Failed to load.</p>
              <button onClick={onReroll} className="mt-2 rounded border border-white/20 px-2 py-1 text-[11px] hover:bg-white/5">
                Retry
              </button>
            </div>
          </div>
        ...
        <div className="absolute inset-x-0 top-0 flex items-center justify-end gap-1.5 bg-gradient-to-b from-black/70 to-transparent p-2 opacity-0 transition group-hover:opacity-100">
          <a
            ...
            className="rounded-md border border-white/15 bg-black/50 p-1.5 text-white/80 backdrop-blur hover:bg-black/70 hover:text-white"
            title="Download / open"
          >
          ...
          <button
            onClick={onRemove}
            className="rounded-md border border-white/15 bg-black/50 px-2 py-1 text-[10px] text-white/70 backdrop-blur hover:bg-black/70 hover:text-white"
            title="Remove"
          >
            ✕
          </button>
      ...
      <div className="border-t border-white/[0.06] p-3">
        <p className="line-clamp-2 text-xs text-white/70" style={{ fontFamily: "'Manrope', sans-serif" }}>
          {item.prompt}
        </p>
        <div className="mt-2 flex items-center justify-between text-[10px] text-white/40">
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="rounded border border-white/10 bg-white/[0.03] px-1.5 py-0.5 font-mono">{item.model}</span>
            <span className="rounded border border-white/10 bg-white/[0.03] px-1.5 py-0.5 font-mono">{item.ratio}</span>
            <span className="rounded border border-white/10 bg-white/[0.03] px-1.5 py-0.5 font-mono">seed {item.seed}</span>
          </div>
          <button
            onClick={onCopy}
            className="inline-flex items-center gap-1 rounded px-1.5 py-0.5 hover:text-white/80"
            title="Copy prompt"
          >
